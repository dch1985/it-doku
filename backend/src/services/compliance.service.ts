...
async function ensureTenantAccessToDocument(documentId: string, tenantId?: string | null) {
...
export const complianceService = {
  ...
  async listQualityFindings(tenantId?: string | null, documentId?: string) {
    ...
    const documentIds = accessibleDocuments.map((doc) => doc.id);
  ...
  async updateQualityFinding(id: string, changes: QualityFindingUpdatePayload, tenantId?: string | null) {
    ...
    if (tenantId) {
      ...
      await ensureTenantAccessToDocument(finding.documentId, tenantId);
    }
  ...
  async runQualityChecks(documentId: string, tenantId?: string | null) {
    ...
    // Tenant-bound access check to prevent cross-tenant scans/writes.
    // Global documents (tenantId = null) stay visible to all tenants by design.
    if (tenantId && document.tenantId && tenantId !== document.tenantId) {
      throw new ApplicationError('Zugriff auf dieses Dokument ist nicht erlaubt', 403);
    }
