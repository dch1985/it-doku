...
export const automationService = {
  ...
  async approveJob(id: string, tenantId: string | null | undefined) {
    if (!tenantId) {
  ...
  async updateSuggestion(id: string, changes: SuggestionUpdateInput, tenantId: string | null | undefined) {
    if (!tenantId) {
      throw new ApplicationError('Tenant-Kontext erforderlich', 400);
    }
    ...
    if (!suggestion || suggestion.generationJob?.tenantId !== tenantId) {
