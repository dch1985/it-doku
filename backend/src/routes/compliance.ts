...
router.patch('/quality/findings/:id', async (req: Request, res: Response) => {
  try {
    ...
    const hasResolution = body && Object.prototype.hasOwnProperty.call(body, 'resolution');
...
router.patch('/reviews/:id', async (req: Request, res: Response) => {
  try {
    ...
    const hasComments = body && Object.prototype.hasOwnProperty.call(body, 'comments');
    const comments = hasComments ? body?.comments ?? null : undefined;
