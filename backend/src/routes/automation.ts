...
router.post('/jobs/:id/approve', async (req: Request, res: Response) => {
  try {
    const existing = await automationService.getJobWithDetails(req.params.id);
...
router.patch('/suggestions/:id', async (req: Request, res: Response) => {
  try {
    const suggestionRecord = await automationService.getSuggestion(req.params.id);
    if (!suggestionRecord) {
      return res.status(404).json({ error: 'Suggestion nicht gefunden' });
    }

    const parentJob = suggestionRecord.generationJobId
      ? await automationService.getJobWithDetails(suggestionRecord.generationJobId)
      : null;

    if (!parentJob || parentJob.tenantId !== req.tenant?.id) {
      return res.status(404).json({ error: 'Suggestion nicht gefunden' });
    }
