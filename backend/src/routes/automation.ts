...
router.patch('/suggestions/:id', async (req: Request, res: Response) => {
  try {
    ...
    const suggestionContext = await automationService.getSuggestionWithTenantContext(req.params.id);
    if (!suggestionContext || suggestionContext.generationJob?.tenantId !== req.tenant?.id) {
