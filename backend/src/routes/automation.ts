...
router.patch('/suggestions/:id', async (req: Request, res: Response) => {
  try {
    ...
    const scopedSuggestion = await automationService.getSuggestionWithTenant(req.params.id);
    if (!scopedSuggestion || scopedSuggestion.generationJob?.tenantId !== req.tenant?.id) {
