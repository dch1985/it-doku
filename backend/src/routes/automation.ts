...
router.patch('/suggestions/:id', async (req: Request, res: Response) => {
  try {
    ...
    const suggestion = await automationService.getSuggestionWithJob(req.params.id);
    if (!suggestion || suggestion.generationJob?.tenantId !== req.tenant?.id) {
      return res.status(404).json({ error: 'Suggestion nicht gefunden' });
    }

    const updatedSuggestion = await automationService.updateSuggestion(req.params.id, {
      status: body.status,
